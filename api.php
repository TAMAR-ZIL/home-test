<?php
	...
	switch($data){
		...
		case "get_chats":
			...
			$results = mysql_fetch_array($query, [$username, $username, $username, $limit, $offset]);
		...
		case "send_wa_voice_msg":
			...
			basic_log_to_file(["POST" => $_POST, "FILES" => $_FILES]);
			
			$contact_id = $_POST["contact_id"] ?? null;
			$username = $_POST["username"] ?? null;
			
			if(!$username){
				error_log("ERROR voice_msg_no_username");
				echo json_encode(false);
				die();
			}
			
			if(!$contact_id){
				error_log("ERROR voice_msg_no_contact_id");
				echo json_encode(false);
				die();
			}
			
			if(!isset($_FILES["voice"]) || $_FILES["voice"]["error"] !== UPLOAD_ERR_OK){
				error_log("ERROR voice_file_upload_failed");
				echo json_encode(false);
				die();
			}
			...
			$filename = "voice_msg_" . $timestamp . "_" . uniqid() . ".webm";
			$upload_path = "./assets/audio/" . $filename;
			
			// Move uploaded file
			if(!move_uploaded_file($_FILES["voice"]["tmp_name"], $upload_path)){
				error_log("ERROR voice_file_move_failed");
				echo json_encode(false);
				die();
			}
			
			// Get user IDs for message insertion
			$my_contact_id_query = "SELECT `id` FROM users WHERE `username` = ? LIMIT 1";
			$des_username_query = "SELECT `username` FROM users WHERE `id` = ? LIMIT 1";
			...
			if(!$my_contact_id || !$des_username){
				error_log("ERROR voice_msg_user_lookup_failed");
				echo json_encode(false);
				die();
			}
			
			// Insert voice message for sender
			$results1 = mysql_insert("messages", [
				"belongs_to_username" => $username,
				"contact_id" => $contact_id,
				"is_from_me" => 1,
				"msg_type" => "audio",
				"msg_body" => $upload_path,
			]);
			
			// Insert voice message for receiver
			$results2 = mysql_insert("messages", [
				"belongs_to_username" => $des_username,
				"contact_id" => $my_contact_id,
				"is_from_me" => 0,
		...
		case "delete_msg":
			...
			if(!$msg_id || !$username || !$contact_id){
				error_log("ERROR delete_msg missing params");
			...
			$msg = mysql_fetch_array($query, [$msg_id]);
			if(!$msg || !isset($msg[0])){
			...
			// Find both message rows (for sender and receiver)
			$body = $msg["msg_body"];
			$datetime = $msg["msg_datetime"];
			$type = $msg["msg_type"];

			// Mark both as deleted (set type to 'revoked', body to placeholder)
			$update1 = mysql_update("messages", ["msg_type"=>"revoked", "msg_body"=>"הודעה זו נמחקה"], ["row_id"=>$msg_id]);

			// Find the matching message for the other user
			$query2 = "SELECT row_id FROM messages WHERE msg_body = ? AND msg_datetime = ? AND contact_id = ? AND msg_type = ? AND row_id != ? LIMIT 1;";
			$otherMsg = mysql_fetch_array($query2, [$body, $datetime, $contact_id, $type, $msg_id]);
			if(isset($otherMsg[0]["row_id"])){
				$other_id = $otherMsg[0]["row_id"];
				$update2 = mysql_update("messages", ["msg_type"=>"revoked", "msg_body"=>"הודעה זו נמחקה"], ["row_id"=>$other_id]);
			}
